...
###SBATCH -n 10
#SBATCH -t 1:0:0
###SBATCH -o slurm_output/PENTrackOutput.txt
###SBATCH --ntasks-per-node=1
#SBATCH --array=0-3 # run multiple simulations in parallel
#SBATCH --time=1:00:00 # set maximum time simulations are allowed to run (should be as close as possible to the actual running time reported by sacct command)
...
cd ../..
./PENTrack $JOB$ID test/comsolFieldTest/config.in test/comsolFieldTest/out/
